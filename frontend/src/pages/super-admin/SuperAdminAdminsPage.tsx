  ...
  return (
    <div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-full sm:w-fit mb-6 overflow-x-auto">
        {(['pending', 'approved', 'declined', 'all'] as FilterTab[]).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 rounded-lg text-sm font-medium transition whitespace-nowrap ${
              filter === f ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {f.charAt(0).toUpperCase() + f.slice(1)}
            <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${filter === f ? 'bg-gray-100 text-gray-600' : 'text-gray-400'}`}>
              {counts[f]}
            </span>
          </button>
        ))}
      ...
      <div className="space-y-3">
        {filtered.map((admin) => (
          <div key={admin._id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-400 mt-0.5 truncate">{admin.email}</p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      Registered: {new Date(admin.createdAt).toLocaleDateString()}
                      {admin.reviewedAt && ` · Reviewed: ${new Date(admin.reviewedAt).toLocaleDateString()}`}
                    </p>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 text-gray-300 hover:text-red-500 transition p-1.5"
                    title="Delete admin"
